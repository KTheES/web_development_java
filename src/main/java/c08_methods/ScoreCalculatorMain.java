...
//        System.out.println("평균 점수는"+ avgScore+ "점 입니다(메서드 활용)");
        ...
        while(!endOfCalc){      // endOfCals를 추후 true로 바꾸는 조건을 통해 반복문 종료 예정
            System.out.print("점수를 입력하세요(종료하려면 -1을 입력하세요) >>> ");
            score1 =scanner.nextDouble();
            if(score1== -1){
                break;  //switch 문에서 배운 break문으로 : 반복문 종료할 때 쓰임
//                -> 곧장 반복문을 종료시키고자 할 때 사용
//                endOfCalc =true;    //endOfCalc라는 변수를 true로 바꿔줌으로써 반복문 종료  ->
                // 다음 반복문이 실행될 때 boolean 연산자의 true/false를 검증하기 때문에 그 전까지는 그대로 실행됨
            }
...
//            System.out.println("총 점은 "+ totalScore +"점입니다.\n현재 과목의 수는 "+totalSubjects+"개 입니다.");
            // 점수를 입력할 때마다 점수와 평균을 확인하기 위해 while문 내부로 해당 코드를 옮김.
