...
/*
    ...
    함수와 메서드가 동일한 개념이라면 왜 용어를 분리하는가?
        ...
        -> 자바의 경우는 모든 함수들이 특정 클래스에 종속돼있기 때문에 전부 다 메서드에 해당함.
    ...
    미리 입력(메서드 정의) 해놓고 나서, '사진을 찍는다' 라는 명령어를 실행(메서드 호출)시
...
//        // scanner 를 이용한 데이터를 routine이라는 변수에 대입
...
//        call3();
        // 별찍기, 즉 return 값을 출력하기 위해 출력하라는 명령어를 집어 넣어야만 함.
...
//        System.out.println(todayDate);              // 실행 방법 2 : 별도의 변수를 선언하여 메서드의 return 값을 저장 후, 변수를 통해 sout 실행

        System.out.print("당신이 태어난 년도는 언제입니까? 숫자만 쓰세요. >>> ");
        int year = scanner.nextInt();
        System.out.println("당신이 태어난 월은 언제입니까? 숫자만 쓰세요. >>>");
        int month = scanner.nextInt();
        System.out.println("당신이 태어난 일은 언제입니까? 숫자만 쓰세요. >>>");
