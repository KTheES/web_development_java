...
/*
    Lombok
        자바에서 반복적으로 작성해야 하는 코드(boilerPlate code :보일러플레이트 코드)
        자동으로 생성해주는 라이브러리

        장점 :
            코드의 가독성을 높일 수 있다 -> 주석을 통한 애너테이션을 붙여 코드 생성을 제어 가능
            특히 스크롤링해서 setter가 있는지 없는디, 특정한 field에 대한 setter/getter의
            유무 등을 고려할 필요가 없이 @ 파트만 보더라도 개락적인 코드 흐름을 알 수 있음
        ...
        5.@EqualsAndHashCode
            -equals()와 hashCode()메서드를 자동으로 생성 -> 둘이 세트로 만들어야 하기 때문에 묶어 사용
