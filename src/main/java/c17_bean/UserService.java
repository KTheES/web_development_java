...
public class UserService {
    /*
        ...
        Entity 클래스 : 주로 데이터베이스 테이블(표)와 매핑(매칭) 되는 클래스
        일반적으로 애플리케이션 데이터 모델을 정의하며, 데이터베이스와의 상호작용을 위해 사용됨 -> DBMS

        주요 특징
        데이터베이스 테이블 매핑 : Entity 클래스는 데이터베이스 테이블의 구조와 일피
            -> 지난 시간에 +--------+ 이 형태로 그렸던 표와 관련있습니다,
        필드와 컬럼 매핑 @ : 클래스의 필드는 데이터베이스 테이블의 컬럼과 매핑됩니다.(마찬가지로 지난 시간 수업)
        식별자 : @Id 애너테이션을 사용하여 기본 키(prime key)를 지정합니다 -> Spring 관련pK
        애너테이션 사용 : @Entity , @Table, @Colume 등의 애너테이션 서용 -> Lombok 과 관련
        ...
        해시코드(HashCode) : 객체의 고유한 정수 값을 반환하는 메서드
            ...
            1. 기본 구현 : 기본적으로 Object 클래스의 hashCode() 는 객체의 메모리 주소를 기반으로 한
                정수 값을 반환 -> 대부분 클래스에서 '재정의'됨
