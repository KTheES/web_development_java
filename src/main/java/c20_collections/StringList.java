...
/*
    ...
    1.Java Collection Framework란?
        자바에서 데이터를 효율적으로 저장하고 관리하기 위한 자료구조와 알고지름을 제공하는
        라이브러리
        컬렉션 프레임워크를 사용하면 다양한 자료구조를 쉽게 구현하고, 데이터를 추가, 삭제
        정렬, 검색 등의 작업을 간편하게 수정 가능
    ...
    2. 컬렉션의 주요 특징
        ...
        3) 제네릭 지원
            -타입 안정성을 제공하며 명시적 형 변환이 필요 없음
        4) 표준 인터페이스 제공
            -컬렉션은 공통 인터페이스(List, Set, Map)를 통해 일관된 프로그래밍 방식을 제공

    3. Java Collection 구조
        컬렉션 프레임워크는 크게 세 가지 주요 '인터페이스'를 중심으로 구별함.
        1) List
        ...
        3) Map
            - 키(Key) 와 값(Value)의 쌍으로 디이터를 저장
            - 키는 중복을 허용하지 않지만 값은 중복이 가능
            - 구현체 : HashMap, LinkedHashMap, TreeMap, HashTable

        이후 부분은 List는 StringList.java에, Set은 StringList.java에 서술합니다.
...
public class StringList {
    public static void main(String[] args) {
        ...
        //List의 데이터 삽입
        strList.add("java");     //list는 .add() 메서드를 통해 하나씩 요소를 삽입
        strList.add("python");
        strList.add("C#");
