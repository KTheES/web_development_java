...
public class Main {
    public static void main(String[] args) {
        ...
        //현재 코드를 작성한 바에 따르면 객체를 centralControl에 추가했을 때만 콘솔창에 어떤 클래스의 객체가 추가되었는지
        // 확인하는 것만 가능한 상태입니다.
...
//        centralControl.displayInfo3();
        ...
        //displayInfo()3의 경우에는 배열 전체를 Arrays.toString()을 이용하여 배열 자체를 출력할 수 있게끔 작성함.
        // 이상의 경우에 배열 내부에 있는 element가 무엇이냐에 따라서 표기 방식이 다르게 됨.
        // 예를 들어 배열 내부의 element가 기본 자료형이라면 그 값이 그대로 String으로 바뀌어서 쓰이게 되겠지만,
        // 현재와 같은 경우는 'Power클래스의 서브 클래스의 객체'가 element이기 때문에 참조 자료형이라 주소값이
        // String으로 바뀌어서 콘솔에 출력되게 됩니다.


        //배열의 각 요소들을 on시키려면 어떻게 작성해야 할까요? ->  그래서 매번 메서드들을 확인해서 그 구조가 어떻게 되는지
        //확인할 필요가 있습니다. 해당 경우 powerOn() 메서드 내에 이미 반복문이 위치해있기 떄문에
