...
/*
    ...
    참조 자료형 캐스팅(Reference Type Casting)
        객체 지향 프로그램(OOP)에서 중요한 요소 중 하나입니다.
        ...
        4. 업캐스팅과 다운 캐스팅의 활용
            캐스팅은 주로 다형성(Polymorphism)을 구현할 떄 사용함. 상위 클래스 타입으로 객체를 처리하면서도
        ...
        5. 캐스팅의 장단점
            ...
            단점 :
                다운 캐스팅의 위험성 : 잘못 다운 캐스팅을 하게 됩니다.
                    예를 들어 상위 클래스 A 와 하위 클래스 B, C가 있다고 가정했을 때, B의 객체인 b를 a로
                    업캐스팅한 후, C로 다우 캐스팅하는 문제가 생길 수 있어 주의가 필요함.

                    복잡성 증가 : 코드 가독성이 떨어질 수 있음 -> 이렇게 바꿨다가 저렇게 바꿨다가 하기 때문인데
                        특히 업캐스팅의 경우 암시적으로 일어나기 때문에 어디서 바뀌었는지 확인은 못하고 있다가
                        갑자기 다운캐스팅이 튀어나오는 경우에 초심자들이 코드를 읽는게 힘겨울 수 있습니다.
    ...
    instanceof 연산자 : Java에서 객체의 실제 타입을 검사하는 데 사용(어떤 클래스의 객체인지를 확인하는 용도)
        ...
        형식 :
            ...
            반환값은 object가 ClassName의 인스턴스이거나 ClassName의 하위 클래스의 인스턴스인 경우 true 반환,
            아니면 false를 반환함.
 */
public class Main {
...
//        animal2.fetch();          // animal1때와 동일하게 호출 불가능함.
        ...
        boolean result6 =(animal3 instanceof Animal);
        System.out.println(result5);
        System.out.println(result6);

        // 다운 캐스팅 관련 수업
        Dog dog2 = (Dog) animal2;   // 다운캐스팅의 경우 명시적으로 쓰여야하기 때문에
                                    // 이전에 배운 형변환 방식인 '(클래스명)'을 사용해야 합니다.
                                    // 이전까지는 double pi = 3.14;
                                    // int three =(int) pi; 와 같은 방식으로 작성했지만
                                    // 이제는 클래스와 객체 단위에서 생각할 필요가 있습니다.
        dog2.makeSound();           // 이제는 대놓고 Dog클래스의 객체이기 때문에 재정의된 makeSound() 호출
...
2 actionable tasks: 2 executed
            ...
            그 이유는 :
                애초에 Animal로 만든 것을 아무런 변형 없이 강제적으로 다운캐스팅을 시도했기 때문입니다.
        ...
        System.out.println("animal4의 검증 부분입니다.");
